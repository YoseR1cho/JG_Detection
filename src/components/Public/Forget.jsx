    ...
    return (
        <>
            <form className={styles.form} onSubmit={handleSubmit(handleRegister)}>
                ...
                {errors.verification && (<p className={styles.error}><ExclamationCircleOutlined className={styles.erroricon}/>请输入验证码</p>)}
                <div>
                    <span className={styles.switch} onClick={()=>setLoginState(1)}>去登录</span>
                    <span className={styles.forget} onClick={()=>setLoginState(2)}>注册</span>
                </div>
                <Button type='submit' loading={loading}>找回</Button>
            </form>
        </>
    );
};
