...
import display from '@/assets/display1.jpg'
...
const WorkOrderList = () => {
    ...
    const data = Array.from({
        ...
        description: <span><div>{workOrderList[i].advice}</div></span>,
        index: i,
    ...
    return (
        <List
            ...
            dataSource={filterOpt ? data.filter(item=>item.status===filterOpt):data}
            renderItem={(item) => (
                <List.Item
                    extra={
                        <img
                            ...
                            height={150}
                            alt="logo"
                            src={filterOpt==='维修完成，待专家复审'?item.repairedUrl:item.originalUrl}
                        />
                    }
                >
                    <List.Item.Meta
                        //   avatar={<Avatar src={item.avatar} />}
                        title={<a className={styles.title}>
                            <span>派单时间：</span><span className={styles.desc}>{item.createTime}</span>
                            <span className={styles.desc} style={{color:(item.level==='三级风险' && 'red') || (item.level==='二级风险' && 'orange') || (item.level==='一级风险' && '#008f00')}}>{item.level}</span>
                            <span style={{color:item.status==='已保存'?'deepskyblue':'#008c00'}} className={styles.status}>{(item.status==='已保存' && '待接单')||(item.status==='已接单' && '已接单') || (item.status==='已处理' && '已处理')}</span>
                            {item.ifUrgent===1 && <div className={styles.urgent} style={{color:'red'}}>需紧急处理</div>}
                        </a>}
                    />
                    ...
                    <List.Item.Meta
                        ...
                        description={<div><span>派单人员：</span><span style={{marginRight:'1rem'}}>{item.inspector}</span></div>}
                    />
                    <span>
