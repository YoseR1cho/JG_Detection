  ...
  return (
      <List
          ...
          <div className={styles.header}>
            ...
            <Grade_Selector level={gradeLevel} setLevel={setGradeLevel}/>
          </div>
      }
          loading={loading}
          dataSource={gradeLevel?data.filter(item=>item.level===gradeLevel):data}
          renderItem={(item) => (
              <List.Item
                  key={item.title}
                  actions={[
              ...
              >
                <List.Item.Meta
                    ...
                    title={<a className={styles.title}>
                        <span>隐患类型：</span><span className={styles.desc}>{item.defectType}</span>
                        <span>风险等级：</span><span className={styles.desc} style={{color:(item.level==='三级风险' && 'red') || (item.level==='二级风险' && 'orange') || (item.level==='一级风险' && '#008f00')}}>{item.level}</span>
                    </a>}
                    description={item.area}
                />
                  <List.Item.Meta
                      description={<div><span style={{marginRight:'1rem'}}>{item.inspector}</span><span>{item.createTime}</span></div>}
                  />
                {item.content}
                <span >
